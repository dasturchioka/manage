...
<template>
  <div class="board-element bg-dark-secondary transition py-2 px-4 rounded">
    ...
    <h2 v-show="!showEditInput" class="title mt-2 text-lg flex">
      {{ taskTitle }}
      <AppIconButton @click="handleEditInput" class="transition opacity-0 flex items-center justify-center ml-2">
        <Edit />
      </AppIconButton>
    </h2>
    <form v-show="showEditInput" class="inline-edit-task mt-2 flex items-center">
      <input
        type="text"
